...
int find_duplicate_optimized(int nums[], size_t n_plus_1) {
    // Phase 1: Find the intersection point of the two pointers.
    // Tortoise moves one step, hare moves two steps.
    int tortoise = nums[0];
    int hare = nums[0];

    // Using a do-while loop ensures at least one iteration, which is necessary
    // because tortoise and hare start at the same position.
    do {
