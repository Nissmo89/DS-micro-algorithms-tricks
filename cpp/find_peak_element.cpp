...
// Finds a peak element in an array. A peak element is an element that is strictly greater than its neighbors.
// The function assumes that nums[-1] = nums[n] = -infinity.
// This implies that if the array has only one element, it is a peak.
