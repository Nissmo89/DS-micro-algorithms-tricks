...
// This version uses helper functions for max/min for clarity.
	...
	for low <= high {
		...
		if maxLeft1 <= minRight2 && maxLeft2 <= minRight1 {
			...
			if (m+n)%2 == 0 { // Even number of elements
				return (maxFloat64(maxLeft1, maxLeft2) + minFloat64(minRight1, minRight2)) / 2.0
			} else { // Odd number of elements
				return maxFloat64(maxLeft1, maxLeft2)
			}
